...
# ---------- Helpers ----------
st.set_page_config(page_title="Heart Disease — Cinematic", layout="wide", initial_sidebar_state="auto")
...
try:
    ...
    st.error("Could not load model pipeline `lr_heart_disease_model.pkl`. تأكد الملف موجود في نفس المجلد.")
    st.stop()
...
with tab_home:
    ...
    st.markdown("<h1 style='margin:0'>❤️ Heart Disease Prediction — Cinematic</h1>", unsafe_allow_html=True)
